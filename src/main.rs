...
impl YouTube {
    async fn new() -> anyhow::Result<Self> {
        let downloader = Downloader::with_new_binaries("binary", "out")
            .await?
            .build()
            .await?;
    ...
    fn try_artist_track(video: &Video) -> (String, Option<String>) {
        ...
        if let Some(description) = &video.description
            ...
            && let Some(artist) = video.tags.first()
        {
            return (artist.to_string(), Some(video.title.clone()));
        }
        ...
        if split.len() >= 2 {
            ...
            (video.title.clone(), Some("".to_string()))
        }
    }
            ...
            if let Some(user) = message.from {
                if name.is_empty() {
                    ...
                    if state.data.queue.len() == 12 {
                        ...
                        state.data.add(name, user.full_name());
                    }
                }
            }
        }
            ...
            for link in YouTube::get_link_list(text) {
                ...
                for x in 0..3 {
                    ...
                    if let Ok(download) = download {
                        match download {
                            Ok(video) => {
                                let title = "out/audio.mp4";
                                let (artist, track) = YouTube::try_artist_track(&video);
